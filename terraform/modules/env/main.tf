...
# Maintainers running apps/admin locally (TOKENS_CLOUDRUN_LOCAL_GCLOUD_AUTH)
# send a gcloud user identity token, which must pass Cloud Run IAM on the
# IAM-locked admin service. The handler-level admin allowlist still applies.
resource "google_cloud_run_v2_service_iam_member" "admin_local_invokers" {
  for_each = var.admin_local_invokers

...
Webacy's depeg monitor has live Solana data for USDC, USDT and PYUSD only (verified 2026-09-14). The in-house peg guard (`apps/cloudrun-assets`, job `refresh-peg-guard`, observer `peg_guard`) covers every other curated stablecoin: every 5 minutes (15 on staging) it prices every `currencies` list mint with one Birdeye `GET /defi/multi_price` call, derives a tier from below-peg deviation against a per-mint reference, and feeds the same reconciler the Webacy job uses. Every curated stablecoin with a known peg drives advisories: USD variants (`usd:*`), the EUR, GBP, CHF, SGD, JPY, TRY, BRL, MXN, NGN, ZAR, AUD, MYR and IDR stables, and yield-bearing USD variants. BUIDL (a fund NAV token) has no peg at all and stays `unsupported_peg`.

**Reference kinds** (`peg_guard_latest.reference_kind`, also on tier events and in the API's `pegHealth.referenceKind`)

| kind | mints | `peg_usd` | bands (below reference) |
|---|---|---|---|
| `fixed` | non-yield `usd:*` variants | 1.00 | `ok` at or above -0.5%, `watch` to -1%, `warning` to -3%, `critical` below -3%; `premium` at +2% or more |
| `fx` | EUR/GBP/... stables (`eur:*` and the standalone symbols) | the CoinGecko-implied USD value of one peg unit, refreshed every run | same bands as `fixed`, measured against the rate |
| `high_water` | yield-bearing `usd:*` variants (`kind = 'yield'`: USDY, sUSD, syrupUSDC, USD*, legacyUSD*) | the token's own ratcheting high-water price | `ok` at or above -1%, `watch` to -2%, `warning` to -5%, `critical` below -5%; never `premium` |

- Fiat rates: one CoinGecko `simple/price` call per run (`usd-coin` and `tether` quoted in USD plus each currency; `usd_per_unit = price_usd / price_ccy`, tether cross-checks usd-coin and a currency whose two implied rates disagree by more than 1% is dropped for the run). The call is skipped when no fx-pegged mint is in the run. Rates are stored in `peg_fx_rates_latest`; on a CoinGecko failure the run logs `peg_guard_fx_fetch_failed{status, message}` (`provider: 'coingecko'`, so the existing external-provider error rules pick it up) and judges against the stored rate for up to 6 hours (`fxStaleMs`, 1 minute to 48 hours). Success logs `peg_guard_fx_rates_refreshed{count, currencies}`. The worker's `COINGECKO_API_KEY` is reused; the client works without it at the public rate limit.
- High-water ratchet (`reference_usd`, `reference_updated_at`): the mark seeds at `max(1.00, price)` on the first observation with DEX liquidity at or above the floor, and afterwards only rises, only on observations at or above the liquidity floor, and by at most 0.1% per elapsed day (`referenceMaxDailyRisePct`, 0 to 5; yield never accrues faster), so a thin-market wick cannot inflate it. It never falls. A run that cannot resolve the reference (no price, thin liquidity) keeps the stored mark: both the row builder and the upsert (`COALESCE`) carry it forward. A yield token already mid-collapse at first sight has no history to compare against; accepted.
- The advisory reason for fx rows reads "... below its EUR peg (1 EUR = $1.1556) ..."; for high-water rows "... below its recent high of $1.1400 ... (yield-bearing token; measured against its own price history)". The token page shows "Holding value" instead of "On peg" for a yield token at or above its mark, "vs $1.1400 recent high" as the price line, and "vs EUR peg ($1.1556)" for fx pegs; the admin curation tooltip names the reference kind.
...
- Signed deviation from the reference (see the table above). Only below-reference deviation counts.
- Liquidity floor: below $100k of DEX liquidity (or unknown liquidity) the observation is recorded with `ok = false` and `error_message = 'thin_liquidity'`; it neither sets nor clears. A Birdeye price whose last trade is older than 6 hours is `stale_price` (quiet stablecoins trade less than hourly and their last price is still evidence); no price anywhere is `no_price`. A failed observation keeps the previous tier and streak in `peg_guard_latest` and leaves `last_ok_at` alone, so the reconciler sees it as stale after 30 minutes.
- Reference issues, checked before the price: `unsupported_peg` (no known peg), `no_fx_rate` (fx peg with no stored or fetched rate for its currency), `stale_fx` (the only rate is older than `fxStaleMs`; the mint is not judged and nothing is cleared), `no_reference` (yield token whose mark has never been seeded because no liquid price has been seen). A mint whose reference resolved this run, or that has a previous successful evaluation, is "tracked" and goes to the reconciler; the rest are recorded only.
...
| `price_fetch_failed` | Birdeye failed and fewer than half of the tracked mints got a fallback price fresher than `priceStaleMs` (`usd_mints` and `tracked_mints` in the log line are the same count; the first name predates fx references) | observations written, no advisory action; the run is `ok: false` only when no fallback price was usable at all |
| `mass_action` | more than `maxActionsPerRun` (5) actions due | criticals first, clears last, the rest deferred to the next run |

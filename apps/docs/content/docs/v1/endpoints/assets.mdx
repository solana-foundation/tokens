...
// See "Advisories" on the Asset by ID page for semantics.
...
Operators can flag a variant mint with an advisory (`caution`, `compromised`, or `blocked`). On this page it shows up in three ways:

- Every `primaryVariant` and `variants[]` element carries `advisory` (`null` when not flagged), and every search/curated row carries `advisories: AssetAdvisoryEntry[]` (`[]` when none, including variants the row itself omitted).
- `compromised` and `blocked` variants are never chosen as `primaryVariant` under any `primaryVariantStrategy` while another eligible variant exists, and are excluded from `/v1/assets/trending`.
- `blocked` variants are omitted from `/v1/assets/search` and `/v1/assets/curated` entirely (a canonical asset whose only variant is blocked disappears from those lists; singleton tokens are dropped). They remain reachable through `GET /v1/assets/:assetId`, which never 404s a flagged mint.

Treat `compromised` and `blocked` as do-not-trade. Full semantics, the TypeScript shapes, and propagation timing are on [Asset by ID → Advisories](/docs/v1/endpoints/asset-by-id#advisories).
...
interface AssetsSearchResponse {
    ...
    results: Array<{
        ...
        advisories: AssetAdvisoryEntry[]; // always present; [] when no variant is flagged
        primaryVariant: null | {
            variantId: string;
            mint: string;
            kind: string;
...
- Rows carry `advisories` and `primaryVariant.advisory` (see [Advisory-flagged assets](#advisory-flagged-assets)). Variants flagged `blocked` are omitted from search results, both from canonical assets’ variant sets and from singleton tokens; `compromised` and `blocked` variants are never selected as `primaryVariant`.
...
- `asset.primaryVariant.advisory` and every `asset.variantGroups.*[]` element carry `advisory` (`null` when not flagged); `asset.advisories` lists every flagged variant. This endpoint never 404s a flagged mint. See [Asset by ID → Advisories](/docs/v1/endpoints/asset-by-id#advisories).
...
- Mints flagged `compromised` or `blocked` are excluded from the ranked set (a crashed token is exactly what tends to trend, so this is enforced at the source rather than by clients). `caution` mints stay ranked. See [Advisory-flagged assets](#advisory-flagged-assets).
...
- Rows carry `advisories` and `primaryVariant.advisory` (and `advisory` on each `variants[]` element with `variants=all`). Variants flagged `blocked` are omitted from curated results (including `groupBy=mint` rows), and `compromised`/`blocked` variants are never chosen as `primaryVariant`. See [Advisory-flagged assets](#advisory-flagged-assets).
...
- `imageUrl` is curated canonical imagery (including storage-backed URLs when configured). `primaryVariant.market.logoURI` is a best-effort per-token icon from indexed/cache sources and may be `null` (the server may fall back to `imageUrl` when missing). When tokens.xyz holds a first-party copy of the artwork, `logoURI` points at tokens.xyz-controlled storage (a 256px WebP under `storage.googleapis.com/tokens-asset-logos-…/solana/<mint>.webp`) instead of the upstream registry URL, so it never depends on rate-limited IPFS gateways or SVG support.

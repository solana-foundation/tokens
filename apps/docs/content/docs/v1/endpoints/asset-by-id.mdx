...
- For assets with `category: "stablecoin"` only, `asset.primaryVariant` and every variant object under `asset.variantGroups.*[]` carry `pegHealth: { tier, deviationPct, updatedAt, stale } | null`, a compact per-mint view of the Webacy depeg monitor (`null` when that mint is not monitored). The key is absent on every other category. The full block, plus the structural-health grade, lives on `include=risk` and the `/risk-*` routes; see [Stablecoin health](#stablecoin-health).
...
type AdvisorySource = 'admin' | 'webacy_depeg';
...
- `webacy_depeg`: set automatically when the Webacy depeg monitor rates a stablecoin mint `warning` or `critical`. System advisories are **always `caution`**; escalation to `compromised` or `blocked` is a human decision and flips `source` to `admin`. The system clears only its own rows, after the peg has recovered for a cooldown. `reason` follows a fixed template and is not refreshed with live prices; read the live deviation from `pegHealth` instead. `url` is `null` on system rows.
...
Two provider-sourced blocks describe how a stablecoin mint is holding up: a live **peg status** from the Webacy depeg monitor (refreshed on tier-change webhooks, reconciled every few hours) and a slower **structural health** grade (refreshed daily). Both are keyed by mint, both are always present on `risk` payloads, and both are `null` when the mint is not monitored (which is the case for every non-stablecoin today).
...
interface PegHealth {
    provider: 'webacy';
    tier: PegTier; // derived by Webacy from overallRisk: ok <25, watch 25-49, warning 50-69, critical >=70
    overallRisk: number | null; // 0-100, higher = riskier
...
- `premium` is not a depeg: the token trades 2%+ **above** its peg with no risk signals.
...
- A `warning` or `critical` tier normally arrives together with a `caution` advisory whose `source` is `webacy_depeg`. Use the advisory for the banner and `pegHealth` for the live numbers.
- Peg and structural data are provided by [Webacy](https://dd.xyz) (dd.xyz). Attribute it when you display it.

...
- `asset.imageUrl` is curated canonical imagery (storage-backed where configured). `asset.primaryVariant.market.logoURI` is a best-effort per-token icon from indexed/cache sources and may be `null` (the server may fall back to `asset.imageUrl` when missing). When tokens.xyz holds a first-party copy of the artwork, `logoURI` points at tokens.xyz-controlled storage (256px WebP) instead of the upstream registry URL.
...
## Launches: `GET /v1/assets/:assetId/launches`

Coins launched on a launchpad (currently [stonk.fun](https://www.stonkfun.xyz)) using any of this
asset's mints as the **quote** token — e.g. everything launched against USDC for `usd`. Powers the
"Launched on &lt;SYMBOL&gt;" section on asset pages.
...
```ts
interface AssetLaunchesResponse {
    assetId: string;
    total: number; // every visible launch, not just this page — drive "View all" from it
    limit: number;
    launches: Array<{
        mint: string;
        symbol: string | null;
        name: string | null;
        logoURI: string | null;
        quoteMint: string; // which of the asset's mints it launched against
        quoteSymbol: string | null;
        launchpad: string; // e.g. "stonkfun"
        price: number | null;
        ...
        launchedAt: number | null; // unix ms
        graduatedAt: number | null; // unix ms; null while still on the launchpad curve
        externalUrl: string; // the coin's page on the launchpad
        advisory: VariantAdvisory | null; // same shape as the variant `advisory` field
    }>;
    lastUpdatedAt: number | null; // freshest launchpad sync backing this response
}
```
...
- Only launchpads and coins approved by Tokens operators appear, so `launches` is empty for most
  assets and may be empty for a while after a launchpad integration goes live. `total: 0` is a
  normal answer, not an error.
- `blocked` mints are omitted; `caution` / `compromised` launches are returned with `advisory`
  populated. Treat `compromised` as do-not-trade.
- Accepts every asset ref form (`assetId`, alias, mint, `solana-<mint>`). Unknown assets 404.

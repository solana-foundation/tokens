...
type PegReferenceKind = 'fixed' | 'fx' | 'high_water';

interface PegHealth {
    provider: PegProvider; // which observer produced this row; tier semantics differ (see below)
    pegCurrency: string | null; // ISO 4217 code of the peg (USD, EUR, ...); null when the observer does not say
    referenceKind: PegReferenceKind; // what pegUsd is (see "Reference kinds"); older API builds omit it, treat as 'fixed'
    tier: PegTier;
    overallRisk: number | null; // Webacy 0-100, higher = riskier; always null for provider 'tokens'
    deviationPct: number | null; // signed percent from the reference; negative = below it
    priceUsd: number | null;
    pegUsd: number | null; // the resolved reference the tier was judged against (1.00, the fiat rate, or the high-water mark)
...
interface CompactPegHealth {
    ...
    referenceKind?: PegReferenceKind; // omitted by older API builds; treat a missing value as 'fixed'
    tier: PegTier;
...
`pegUsd` is always the resolved reference the tier was judged against, and `referenceKind` says what that reference is:

- `fixed`: a fixed 1.00 per unit of `pegCurrency`. Every Webacy row is `fixed`, as is every USD stablecoin the tokens.xyz peg monitor judges.
- `fx`: a non-USD peg (EUR, GBP, CHF, SGD, JPY, TRY, BRL, MXN, NGN, ZAR, AUD, MYR, IDR). `pegUsd` is the USD value of one unit of `pegCurrency`, implied from CoinGecko's USDC and USDT quotes in that currency and refreshed every peg guard run. A CoinGecko outage reuses the last good rate for up to 6 hours; after that the mint is not judged and its last tier is served with `stale: true` once the observation ages out.
- `high_water`: a yield-bearing USD variant (USDY, syrupUSDC, sUSD, ...) whose price accrues above 1.00 by design, judged against its own recent high rather than 1.00. `pegUsd` is that high-water mark. The reference only rises, only on observations with enough liquidity, and by at most 0.1% per elapsed day, so a thin-market wick cannot inflate it; it never falls.

The web app renders a `high_water` `ok` row as **"Holding value"** (not "On peg") and its `watch` row as "Slipping", because a small dip below the recent high is not a peg wobble. Use the same vocabulary if you surface these rows.

### Guidance

- **Tier semantics depend on `provider` and `referenceKind`.**
    ...
    - `tokens`, `fixed` and `fx`: `tier` is derived from `deviationPct` alone, and only below-reference deviation counts: `ok` at or above -0.5%, `watch` down to -1%, `warning` down to -3%, `critical` below -3%. `overallRisk` is always `null`.
    - `tokens`, `high_water`: wider bands against the token's recent high: `ok` at or above -1%, `watch` down to -2%, `warning` down to -5%, `critical` below -5%. These rows never show `premium`; any deviation at or above zero reads as `ok`.
